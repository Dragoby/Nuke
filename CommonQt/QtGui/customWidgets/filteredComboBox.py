...
class FilteredComboBox(QtWidgets.QComboBox):
    def __init__(self):
        ...
        self.lineEdit().textEdited.connect(lambda text: self.filterModel.setFilterFixedString(str(text)))

    def completerActivated(self, text):
        """
        This ensures that we correctly set the index of the comboBox(self) when the user has entered text.  If the
        text has not been presented then this will do nothing.  If the given text can not be found then nothing will
        happen
    ...
    def setModel(self, model):
        """
        We override the setModel to ensure that the completer is properly linked to the newly updated model
        """
    ...
    def setModelColumn(self, column):
        """
        This assures that all of the columns are updated for all models when the model of the comboBox(self) is updated1
        """
