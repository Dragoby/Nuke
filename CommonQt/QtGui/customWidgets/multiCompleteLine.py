...
class MultiCompleteLine(QtWidgets.QLineEdit):
    ...
    Kwargs:
        ...
        escapeClear (bool|optional): True or False if the text should be cleared when the user presses escape
        separators (list(str)|optional): This is a list of characters to use as separators when determining the values
                                         of the lineEdit(self).  When this is set it will also override and reset the
                                         value of the completion addition
        completionAddition (str|optional): This is the value that will be added to each completion when it is activated
    """
    ...
    def completionAddition(self, value):
        """
        This ensures that the given completion addition is included in the separators.  This is to ensure proper
        formatting of the text so it may be properly parsed to extract the values
        Args:
        ...
        if value is None or re.search(self.separators, value):
            ...
            raise ValueError('The given addition is invalid.  Please ensure that the addition is included in the '
                             'separators')
    ...
    def separators(self, characters):
        """
        This ensures that the values set for the separators is properly formatted.  It will take the given list of
        characters and format them into an regular expression which will be used to separate out and parse the values

        In addition this will also update the completion addition to be a valid character from within the characters
        Args:
    ...
    def values(self):
        """
        This will return all of the values in the current lineEdit(self).  The values are determined by the current
        separators
    ...
    def setCompletionValues(self, values, updateCache=True):
        ...
        Args:
            values (list(str)): This is a list of strings to be used for the auto completion
            updateCache (bool|optional): True or False if the internal cache of completions is to be updated.  This is
                                         only used internally and set to false when we are limiting existing items
        """
    ...
    def setRegEx(self, regex):
        ...
        Args:
            regex (str): This is a formatted regular expression to be used to filter characters on self
        """
    ...
    def _insertCompletion(self, completion, replace=False):
        ...
        self.itemComplete.emit(self.currentWordAction(action=self.actionGet, currentIndex=cursorPosition - 2))
    ...
    def keyPressEvent(self, event):
        """
        This override ensures that certain key shortcuts activate/complete the completer in the way we want it to be

        The Up key will activate the completer if it is not yet shown.  If the completer is shown an the TAB key
        is pressed the completion will replace the current word where enter will just add
        ...
        if self._completer.popup().isVisible():
            ...
            if event.key() in [QtCore.Qt.Key_Tab, QtCore.Qt.Key_Return, QtCore.Qt.Key_Enter]:
                ...
                self._insertCompletion(completion=self._completer.popup().selectedIndexes()[0].data(), replace=replace)
    ...
    def showCompleter(self, text, prefix=None, force=False):
        """
        When triggered this will process and determine if the completer is to be shown and update the prefix if one is
        not provided.  If the completer is set to ignore existing completions then this will also limit the completions
        to items that have not yet already been entered

        Args:
            text (str): This is the entered text which triggered the completer
            prefix (str|None|optional): This is the prefix that is to be used for the completer.  If this is not
                                        provided then it will be collected based on the current cursor position
    ...
    def _updateCompleterItems(self, prefix):
        """
        This will ensure that the prefix is set correctly,  this is in the event that the given prefix is None or if
        the current prefix on the completer does not match the expected prefix
        """
        if prefix is None:
