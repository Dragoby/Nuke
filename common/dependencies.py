...
class Dependencies(object):
    ...
    def _getExpressionDependencies(self, node, nodes=None):
        """
        This will collect all of the expression linked nodes recursively for the given node.  If a given list of nodes
        is provided all of the collected nodes will be added to that set and returned
        Args:
            node (nuke.Node): This is the node in which to collect the expression dependencies for
            nodes (set|optional): This is a set of node to add the collected nodes to and will be returned

        Returns:
            set: This is a set of the collected dependencies or if a set of nodes is given it will be that set with the
                 addition of the collected dependencies
        """
        nodes = nodes or set()
        ...
        for expressionNode in expressionNodes:
            # We don't want to collect the expressions more than once in the event multiple nodes are expression linked
            # to a single node.  We could get into a recursion loop otherwise
    ...
    def _getDependencies(self, node, **kwargs):
        """
        This is used to collect all of the dependencies used by a given node.  This can collect and check both connected
        nodes and expression linked nodes
        Args:
        ...
        for inputId in range(inputs):
            ...
            if recurseGroups:
                try:
                    dependentNodes = dependentNodes.union(common.utilities.allNodes(context=connectedNode))
